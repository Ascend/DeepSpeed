...
class T5BeforeEmbeddingPipeline(torch.nn.Module, ModuleUtilsMixin):
    ...
    def forward(self, inputs):
        ...
        inputs = inputs._replace(
            use_cache=(inputs.use_cache if inputs.use_cache.numel() != 0 else IntTensor([self.config.use_cache]).to(device)),
            output_attentions=(inputs.output_attentions if inputs.output_attentions.numel() != 0 else IntTensor([self.config.output_attentions]).to(device)),
            output_hidden_states=(inputs.output_hidden_states if inputs.output_hidden_states.numel() != 0 else IntTensor([self.config.output_hidden_states]).to(device)),
            return_dict=(inputs.return_dict if inputs.return_dict.numel() != 0 else IntTensor([self.config.use_return_dict]).to(device))
        )
        ...
        mask_seq_length = inputs.past_key_values[0][0].shape[2] + seq_length if inputs.past_key_values.numel() != 0 else seq_length
        ...
        if inputs.attention_mask.numel() == 0:
            inputs = inputs._replace(attention_mask=torch.ones(batch_size, mask_seq_length).to(dtype=torch.int32, device=device))
        if self.config.is_decoder and inputs.encoder_attention_mask.numel() == 0 and inputs.encoder_hidden_states.numel() != 0:
            ...
            inputs = inputs._replace(encoder_attention_mask=torch.ones(batch_size, encoder_seq_length, device=device, dtype=torch.int))
        ...
        inputs = inputs._replace(extended_attention_mask=self.get_extended_attention_mask(inputs.attention_mask, input_shape, inputs.inputs_embeds.device, inputs.attention_mask.dtype))
        ...
        if self.config.is_decoder and inputs.encoder_hidden_states.numel() != 0:
            ...
            if inputs.encoder_attention_mask.numel() == 0:
                inputs = inputs._replace(encoder_attention_mask=torch.ones(encoder_hidden_shape, dtype=torch.int32, device=inputs.inputs_embeds.device))
            inputs = inputs._replace(encoder_extended_attention_mask=self.invert_attention_mask(inputs.encoder_attention_mask, inputs.encoder_attention_mask.dtype))
        else:
    ...
    def get_extended_attention_mask(self, attention_mask: Tensor, input_shape: Tuple[int], device, dtype) -> Tensor:
        if attention_mask.dim() == 3:
            ...
            if self.config.is_decoder:
                extended_attention_mask = self.create_extended_attention_mask_for_decoder(input_shape, attention_mask, device)
            else:
    ...
    def invert_attention_mask(self, encoder_attention_mask: Tensor, dtype) -> Tensor:
        ...
        if dtype == torch.float16:
            ...
            raise ValueError(f'{dtype} not recognized. `dtype` should be set to either `torch.float32` or `torch.float16`')
...
class T5BlockPipeline(T5Block):
    ...
    def forward(self, inputs):
        ...
        if self.is_decoder and inputs.encoder_hidden_states.numel() != 0:
            inputs = inputs._replace(encoder_decoder_position_bias=layer_outputs[4 if inputs.output_attentions[0].item() else 3])
...
def t5_lmhead_forward(layer, inputs):
    ...
    if bias is not None:
        bias = bias.copy()
        bias.data = torch.nn.functional.pad(layer.bias.data,(0,layer.weight.shape[0] - layer.bias.shape[0],),"constant",0,)
...
def create_tokenizer(pretrained_model_name_or_path: str = 't5-small'):
    tokenizer = T5Tokenizer.from_pretrained(pretrained_model_name_or_path)
    tokenizer.add_special_tokens({'bos_token': '<s>', 'mask_token': '<mask>'})
    tokenizer.add_tokens('<decoder>')  # decoder_start_token
    return tokenizer
...
def create_dataset(tokenizer, dataset_dir, mask_prob, random_replace_span, max_seq_length):
    dataset = datasets.load_dataset('wiki_dpr', 'psgs_w100.nq.exact', split='train[:500000]', cache_dir=dataset_dir)
    log_dist('Dataset length: {}'.format(len(dataset)), ranks=[0], level=logging.INFO)
    masking_function_partial = partial(replace_function, tokenizer=tokenizer, corruption_rate=mask_prob,
                                       corruption_span=random_replace_span, max_length=max_seq_length)
    return WikiDataset(dataset, masking_function_partial)


def collate_function(batch: List[Tuple[List[int], List[int]]], pad_token_id: int):
    max_length = 1024
    padded_token_ids = [token_ids + [pad_token_id for _ in range(0, max_length - len(token_ids))] for token_ids, _ in batch]
...
def train(
    ...
    if checkpoint_dir is None and load_checkpoint_dir is None:
        log_dist('Need to specify one of checkpoint_dir or load_checkpoint_dir', ranks=[0], level=logging.ERROR)
        return
    if checkpoint_dir is not None and load_checkpoint_dir is not None:
        log_dist('Cannot specify both checkpoint_dir and load_checkpoint_dir', ranks=[0], level=logging.ERROR)
        return
    if checkpoint_dir:
    ...
    log_dist('Creating Tokenizer', ranks=[0], level=logging.INFO)
    tokenizer = create_tokenizer(tokenizer_name_or_dir)

    log_dist('Creating Datasets', ranks=[0], level=logging.INFO)
    dataset = create_dataset(tokenizer=tokenizer, dataset_dir=dataset_dir, mask_prob=mask_prob,
                             random_replace_span=random_replace_span, max_seq_length=max_seq_length)
    collate_fn = create_collate_fn(tokenizer)
    log_dist('Dataset Creation Done with length: {}'.format(len(dataset)), ranks=[0], level=logging.INFO)
    ...
    for step in range(start_step, num_iterations):
        ...
        step_msg = 'Step: {}, Loss: {}'.format(step, loss.item())
        log_dist(step_msg, ranks=[0], level=logging.INFO)
