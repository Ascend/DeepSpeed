...
class HcclBackend(object):
    ...
    def compressed_allreduce(self,
        ...
        recvbuf_sign = torch.zeros(
            [self.size,
             len(sign_list_packed_tmp[self.rank])],
            dtype=sign_list_packed_tmp[0].dtype, device=sign_list_packed_tmp.device)

        sign_list_packed = [
            sign_list_packed_tmp[idx]
        ...
        server_sign_packed_tmp = torch_npu.npu_sign_bits_pack(compensated_server_m, 1).type(torch.int8)

        recvbuf_sign_server_tmp = torch.zeros(
            [self.size,
             len(server_sign_packed_tmp[0])],
            dtype=recvbuf_sign.dtype, device=server_sign_packed_tmp.device)

        server_sign_packed = [
            server_sign_packed_tmp[0]
        ]

        recvbuf_sign_server = [
            recvbuf_sign_server_tmp[idx]
            for idx in range(self.size)
        ]

        recvbuf_scale_server_tmp = torch.zeros([self.size,
                                                1],
                                               dtype=worker_scale.dtype, device=server_sign_packed_tmp.device)

        recvbuf_scale_server = [
            recvbuf_scale_server_tmp[idx]
            for idx in range(self.size)
        ]
